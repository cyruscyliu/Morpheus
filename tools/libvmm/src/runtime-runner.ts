...
async function main(argv: string[]) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "const forcedGicVersion = process.env.LIBVMM_GIC_VERSION || '';",
      "const extraQemuArgs = (() => {",
      "  try {",
      ...
      'if (blkStorage) {',
      '  const localBlkStorageCandidates = [',
      '    path.resolve(process.cwd(), "blk_storage"),',
      '    path.resolve(process.cwd(), "build", "blk_storage"),',
      '  ];',
      '  const localBlkStorage = localBlkStorageCandidates.find((candidate) => fs.existsSync(candidate)) || "";',
      '  if (!fs.existsSync(blkStorage) && localBlkStorage) {',
      ...
      '  if (rewritten[index] !== "-machine") {',
      '    continue;',
      '  }',
      '  const machineArg = String(rewritten[index + 1] || "");',
      '  if (!forcedGicVersion || !machineArg.startsWith("virt") || !machineArg.includes("virtualization=on") || machineArg.includes("gic-version=")) {',
      '    continue;',
      '  }',
      '  rewritten[index + 1] = `${machineArg},gic-version=${forcedGicVersion}`;',
      '}',
