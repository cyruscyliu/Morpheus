...
test("findRunRoot loads .env and expands MORPHEUS_DATA_ROOT", () => {
  const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "morpheus-runs-viewer-"));
  const repoRoot = path.join(baseDir, "repo");
  const startDir = path.join(repoRoot, "apps", "runs-viewer");
  const dataRoot = path.join(baseDir, "data-root");
  const workspaceDir = path.join(dataRoot, "workspaces", "hyperarm");

  writeFile(
    path.join(repoRoot, ".env"),
    [`MORPHEUS_DATA_ROOT=${dataRoot}`, ""].join("\n"),
  );
  writeFile(
    path.join(repoRoot, "morpheus.yaml"),
    ["workspace:", "  root: ${MORPHEUS_DATA_ROOT}/workspaces/hyperarm", ""].join("\n"),
  );

  const previousDataRoot = process.env.MORPHEUS_DATA_ROOT;
  const previousWorkspacesRoot = process.env.MORPHEUS_WORKSPACES_ROOT;
  delete process.env.MORPHEUS_DATA_ROOT;
  delete process.env.MORPHEUS_WORKSPACES_ROOT;
  try {
    const resolved = findRunRoot({ startDir });
    assert.equal(resolved.workspaceRoot, workspaceDir);
    assert.equal(resolved.runRoot, path.join(workspaceDir, "workflows"));
    ...
    if (previousWorkspacesRoot == null) {
      delete process.env.MORPHEUS_WORKSPACES_ROOT;
    } else {
      process.env.MORPHEUS_WORKSPACES_ROOT = previousWorkspacesRoot;
    }
  }
});
