...
test('libvmm run adds gic-version=3 for virt EL2 QEMU launches', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'morpheus-libvmm-gic-'));
  const source = path.join(root, 'libvmm');
  const microkitSdk = path.join(root, 'microkit-sdk');
  const toolchain = path.join(root, 'toolchain');
  const qemu = path.join(root, 'qemu-system-aarch64');
  ...
  const capturedArgs = path.join(root, 'qemu-args.txt');
  ...
  fs.writeFileSync(
    ...
    [
      ...
      '\t@true',
      'qemu:',
      '\t$(QEMU) -machine virt,virtualization=on,secure=off -serial mon:stdio',
      '',
    ].join('\n'),
    'utf8',
  );
  ...
  while (!fs.existsSync(capturedArgs) && Date.now() - start < 5000) {
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 50);
  }
  assert.equal(fs.existsSync(capturedArgs), true);
  assert.match(fs.readFileSync(capturedArgs, 'utf8'), /-machine virt,virtualization=on,secure=off,gic-version=3/);
