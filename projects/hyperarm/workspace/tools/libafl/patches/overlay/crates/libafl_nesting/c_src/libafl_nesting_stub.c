...
static bool write_input_snapshot(const uint8_t *data, size_t len) {
  FILE *fp = fopen("/root/morpheus-qemu-input.bin", "wb");
  if (!fp) {
...
static bool launch_l2(const uint8_t *data) {
  ...
  if (pid == 0) {
    setenv("MORPHEUS_QEMU_INPUT_PATH", "/root/morpheus-qemu-input.bin", 1);
  ...
  if (wait_ret == 0) {
    ...
    log_file_state("/root/launch-l2.stdout", "launch-l2.stdout");
    log_file_state("/root/launch-l2.stderr", "launch-l2.stderr");
    log_file_state("/root/launch-l2.marker", "launch-l2.marker");
    log_file_state("/root/morpheus-qemu-trace.log", "morpheus-qemu-trace.log");
    log_file_state("/root/morpheus-nqc2.trace", "morpheus-nqc2.trace");
    FILE *stdout_fp = fopen("/root/launch-l2.stdout", "rb");
    FILE *stderr_fp = fopen("/root/launch-l2.stderr", "rb");
    if (stdout_fp) {
