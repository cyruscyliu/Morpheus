...
void getBlockedNodes(SVFIR *pag) {
  ...
  if (const auto *value = getLLVMValue(LCU);
      value && value->hasName() && LCU->getValueName() == "llvm.compiler.used") {
  ...
  for (const auto &CallEdge : pag->getSVFStmtSet(PAGEdge::Call)) {
    auto SVFCallee = LLVMUtil::getCallee(dyn_cast<CallBase>(
        getLLVMValue(dyn_cast<CallPE>(CallEdge)->getCallSite())));
    if (!SVFCallee) {
      continue;
    }
    auto callee = SVFCallee;
    Callees[callee]++;
    CalleeNodes[callee].insert(CallEdge->getDstID());
  }
  for (auto Callee : Callees) {
    if (Callee.second / (Callee.first->arg_size() + 1) >
        static_cast<size_t>(baseNum * 5)) {
      if (Callee.first->getName().find('.') == string::npos ||
          Callee.second / (Callee.first->arg_size() + 1) >
              static_cast<size_t>(baseNum * 10)) {
  ...
  for (const auto &RetEdge : pag->getSVFStmtSet(PAGEdge::Ret)) {
    auto SVFCallee = LLVMUtil::getCallee(dyn_cast<CallBase>(
        getLLVMValue(dyn_cast<RetPE>(RetEdge)->getCallSite())));
    if (!SVFCallee) {
      continue;
    }
    auto callee = SVFCallee;
  ...
  for (auto ret : Rets) {
    if (ret.second > static_cast<unsigned int>(baseNum * 5)) {
      if (ret.first->getName().find('.') == string::npos ||
          ret.second > static_cast<unsigned int>(baseNum * 10)) {
  ...
  for (u32_t i = 0; i < pag->getNodeNumAfterPAGBuild(); i++) {
    auto node = pag->getGNode(i);
    if (node->getIncomingEdges(PAGEdge::Store).size() >
        static_cast<size_t>(baseNum * 50)) {
      BlockedNodes.insert(i);
    }
    if (node->getOutgoingEdges(PAGEdge::Store).size() >
        static_cast<size_t>(baseNum * 10)) {
      BlockedNodes.insert(i);
    }
    if (node->getOutgoingEdges(PAGEdge::Copy).size() >
        static_cast<size_t>(baseNum * 15)) {
      BlockedNodes.insert(i);
    }
    if (node->getOutgoingEdges(PAGEdge::Load).size() >
        static_cast<size_t>(baseNum * 5)) {
      BlockedNodes.insert(i);
    }
  }
  ...
  for (auto node : Call2Ret) {
    if (node.second.size() > static_cast<size_t>(baseNum)) {
...
string getStructName(StructType *sttype) {
  ...
  if (origin_name.find(".anon.") != string::npos) {
    const auto fieldNum =
        llvmModuleSet()->getSVFType(sttype)->getTypeInfo()->getNumOfFlattenFields();
    auto stsize = getTypeSizeInBytes(sttype);
  ...
  if (deAnonymous) {
    if (deAnonymousStructs.find(sttype) != deAnonymousStructs.end()) {
      ...
      const auto fieldNum = llvmModuleSet()
                                ->getSVFType(sttype)
                                ->getTypeInfo()
...
void addSVFAddrFuncs(SVFModule *svfModule, SVFIR *pag) {
  for (auto F : svfModule->getFunctionSet()) {
    auto funcnode = pag->getGNode(getValueNode(F));
    addrvisited.clear();
    if (checkIfAddrTaken(pag, funcnode)) {
      type2funcs[printType(F->getType())].insert(const_cast<Function *>(F));
    }
  }
}
...
StructType *ifPointToStruct(const Type *tp) {
  if (tp && tp->isPointerTy() && (tp->getNumContainedTypes() > 0)) {
    auto elemTy = LLVMUtil::getPtrElementType(dyn_cast<PointerType>(tp));
    if (auto ret = dyn_cast<StructType>(elemTy)) {
      return ret;
    } else if (auto ret =
                   dyn_cast<ArrayType>(elemTy)) {
...
void handleAnonymousStruct(SVFModule *svfModule, SVFIR *pag) {
  ...
  for (auto &moduleRef : svfModule->getLLVMModules()) {
  ...
  for (auto const edge : pag->getSVFStmtSet(PAGEdge::Gep)) {
    if (auto callinst = dyn_cast_or_null<CallInst>(getLLVMValue(edge->getValue()))) {
...
long regularStructVisit(StructType *sttype, s32_t idx, PAGEdge *gep) {
  // ret byteoffset
  long ret = 0;
  const auto stinfo = llvmModuleSet()->getSVFType(sttype)->getTypeInfo();
  ...
  for (u32_t i = 0; i < lastOriginalType; i++) {
    if (stinfo->getOriginalElemType(i)) {
      auto rtype = getLLVMType(const_cast<SVFType *>(stinfo->getOriginalElemType(i)));
      ret += getTypeSizeInBytes(rtype);
    }
  }
  // Check if completed
  if (idx - lastOriginalType >= 0) {
    auto embType =
        getLLVMType(const_cast<SVFType *>(stinfo->getOriginalElemType(lastOriginalType)));
...
StructType *gotStructSrc(PAGNode *node,
  ...
  for (auto nxt : node->getIncomingEdges(PAGEdge::Copy)) {
    if (auto nxtType = nxt->getSrcNode()->getType()) {
      if (auto elemType = getPointeeType(getLLVMType(nxtType))) {
  ...
  for (auto const edge : pag->getSVFStmtSet(PAGEdge::Gep)) {
    ...
    if (auto callinst = dyn_cast_or_null<CallInst>(getLLVMValue(edge->getValue()))) {
      ...
      if (auto type = edge->getSrcNode()->getType()) {
        if (auto elemType = getPointeeType(getLLVMType(type))) {
                ...
                if (gepstmt->isVariantFieldGep()) {
                  ...
                  gep2byteoffset[edge] =
                      varStructVisit(const_cast<GEPOperator *>(
                          dyn_cast<GEPOperator>(getLLVMValue(edge->getValue()))));
                }
              } else {
...
void processCastSites(SVFIR *pag, SVFModule *mod) {
  ...
  for (auto llvmfunc : mod->getFunctionSet()) {
      ...
      for (auto &inst : bb) {
        if (auto call = dyn_cast<CallInst>(&inst)) {
          if (call->getCalledFunction() &&
            ...
            string dststr = "dst";
            auto dst_pg = pag->getGNode(getValueNode(Dst));
            ...
            string srcstr = "src";
            auto src_pg = pag->getGNode(getValueNode(Src));
      ...
      for (auto &inst : bb) {
        if (auto callinst = dyn_cast<CallInst>(&inst)) {
          if (callinst->isIndirectCall()) {
            callinstsmap[to_string(getValueNode(callinst))] =
                const_cast<CallInst *>(callinst);
          }
        }
      }
    }
...
void setupDependence(SVFIR *pag, SVFModule *mod) {
  for (auto llvmfunc : mod->getFunctionSet()) {
    for (auto &arg : llvmfunc->args()) {
      if (auto pagnodenum = getValueNode(&arg)) {
      ...
      if (argsize == callee->arg_size()) {
        for (unsigned int i = 0; i < argsize; i++) {
          if (hasValueNode(
              ...
              hasValueNode(callee->getArg(i))) {
            const auto real = getValueNode(
                callinst.first->getArgOperand(i)->stripPointerCasts());
            const auto formal = getValueNode(callee->getArg(i));
              ...
              for (auto &inst : bb) {
                if (auto retinst = dyn_cast<ReturnInst>(&inst)) {
                  if (retinst->getNumOperands() != 0 &&
                    ...
                    const auto retval = getValueNode(retinst->getReturnValue());
                    Ret2Call[retval].insert(getValueNode(callinst.first));
                    Call2Ret[getValueNode(callinst.first)].insert(retval);
                  }
                }
              }
            }
  ...
  for (auto edge : pag->getSVFStmtSet(PAGEdge::Copy)) {
    if (auto srcType = edge->getSrcNode()->getType()) {
      if (auto dstType = edge->getDstNode()->getType()) {
        if (srcType != dstType) {
          auto *srcLLVMType = getLLVMType(const_cast<SVFType *>(srcType));
          auto *dstLLVMType = getLLVMType(const_cast<SVFType *>(dstType));
          castmap[srcLLVMType].insert(dstLLVMType);
          castmap[dstLLVMType].insert(srcLLVMType);
        }
      }
    }
  }
...
bool checkIfMatch(const CallInst *callinst, const Function *callee) {
  bool typeMatch = true;
  if (callinst->getCalledOperand()->getType()) {
      ...
      for (unsigned int i = 0; i < callinst->arg_size(); i++) {
        ...
        if (!checkTwoTypes(icallargType, calleeargType, castmap)) {
          if (icallargType->isPointerTy() && calleeargType->isPointerTy()) {
            auto *icallpto = getPointeeType(icallargType);
            auto *calleepto = getPointeeType(calleeargType);
            if (icallpto && calleepto &&
              ...
              break;
               ...
               callee->getReturnType()->isPointerTy()) {
      auto *icallpto = getPointeeType(callinst->getType());
      auto *calleepto = getPointeeType(callee->getReturnType());
      if (icallpto && calleepto &&
          ((icallpto->isStructTy() && !calleepto->isStructTy()) ||
           (!icallpto->isStructTy() && calleepto->isStructTy()))) {
        typeMatch = false;
