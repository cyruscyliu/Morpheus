...
if test -e $TOOLDIR/usr/bin/qemu-aarch64-static && test -e $TOOLDIR/usr/bin/qemu-system-aarch64 ; then
	echo "Managed QEMU tools already exist, skipping tool bootstrap!"
	exit 0
fi
