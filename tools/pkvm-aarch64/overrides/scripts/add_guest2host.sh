...
NBD_AVAILABLE=1
if ! modprobe nbd max_part=8 >/dev/null 2>&1; then
	NBD_AVAILABLE=0
fi
...
if [ "$NBD_AVAILABLE" = "0" ]; then
	echo "Restricted environment: skipping host image mutation"
	mkdir -p "$OUTDIR/guest"
	cp -f "$GUESTIMG" "$OUTDIR/guest/" || true
	if [ -f "$GUESTIMG2" ]; then
		cp -f "$GUESTIMG2" "$OUTDIR/guest/" || true
	fi
	cp -f "$GUESTKERNEL" "$OUTDIR/guest/" || true
