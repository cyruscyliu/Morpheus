...
{"schemaVersion":1,"tool":"pkvm-aarch64","command":"exec","status":"running","run_dir":"${run_dir}","log_file":"${run_dir}/stdout.log","pid":${pid},"timeout_pid":${timeout_pid:-null},"timeout_seconds":${timeout_seconds},"platform":"${platform}","detached":true}
EOF
  cat > "${result_file}" <<EOF
{"details":{"pid":${pid},"detached":true,"run_dir":"${run_dir}","log_file":"${run_dir}/stdout.log"}}
EOF
...
cd "${source_dir}"
if [ "${timeout_seconds}" != "0" ]; then
  timeout --preserve-status "${timeout_seconds}s" "${make_cmd[@]}" || exit_code="$?"
else
  "${make_cmd[@]}" || exit_code="$?"
fi
cd "${work_dir}"
...
if [ "${exit_code}" = "124" ]; then
  cat > "${manifest_file}" <<EOF
{"schemaVersion":1,"tool":"pkvm-aarch64","command":"exec","status":"timeout","run_dir":"${run_dir}","log_file":"${run_dir}/stdout.log","pid":null,"timeout_seconds":${timeout_seconds},"platform":"${platform}","detached":false}
EOF
  cat > "${result_file}" <<EOF
...
cat > "${manifest_file}" <<EOF
{"schemaVersion":1,"tool":"pkvm-aarch64","command":"exec","status":"success","run_dir":"${run_dir}","log_file":"${run_dir}/stdout.log","pid":null,"platform":"${platform}","detached":false}
EOF
cat > "${result_file}" <<EOF
